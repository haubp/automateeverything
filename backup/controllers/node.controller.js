...
async function addNodeCommand(req, res) {
  ...
  await nodeCommandsModel.addNodeCommand(
    req.body.node_id_list,
    req.body.command
  );
  res.status(200).json({});
}
...
async function reportNodeResult(req, res) {
  ...
  if (fs.existsSync(path.join(__dirname, "..", "public", "user_" + user_id))) {
    console.log(
      `Save result from node ${req.query.node_id} into ${user_id} storage`
    );
...
async function uploadAgentLog(req, res) {
  // Query user_id from node_id
  if (!req.query.node_id) {
    res.status(404).json({ status: "Invalid node_id" });
    return;
  ...
  if (fs.existsSync(path.join(__dirname, "..", "public", "user_" + user_id))) {
    ...
    fs.writeFileSync(
      path.join(__dirname, "..", "public", "user_" + user_id, "agent_log.zip"),
...
async function uploadAutoLog(req, res) {
  // Query user_id from node_id
  if (!req.query.node_id) {
    res.status(404).json({ status: "Invalid node_id" });
    return;
  ...
  if (fs.existsSync(path.join(__dirname, "..", "public", "user_" + user_id))) {
    ...
    fs.writeFileSync(
      path.join(__dirname, "..", "public", "user_" + user_id, "auto_log.txt"),
      req.body
    );
  } else {
    console.log(`${user_id} storage not found`);
