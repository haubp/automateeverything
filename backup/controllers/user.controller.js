...
async function updateTemplate(req, res) {
  ...
  if (
    fs.existsSync(
      path.join(__dirname, "..", "public", "user_" + req.query.user_id)
    )
  ) {
    console.log(
      `Save result from node ${req.query.node_id} into ${req.query.user_id} storage`
    );
    ...
    fs.writeFileSync(
      path.join(
        ...
        "..",
        "public",
        "user_" + req.query.user_id,
        "template.json"
      ),
      nodeData
    );
  } else {
    console.log(`${req.query.user_id} storage not found`);
  }
  res.status(200).json({});
}
