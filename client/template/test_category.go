...
func (c *TestCategory) InitContext(a fyne.App, w fyne.Window) {
	...
	c.W = w
	c.A = a
	c.Widget = container.New(
		layout.NewHBoxLayout(),
		canvas.NewText(" "+c.TestCategoryName, color.RGBA{0xD8, 0xD8, 0xD8, 1}),
		layout.NewSpacer(),
...
func (c *TestCategory) DeleteTestGroupByIndex(i int) {
	c.TestCategoryGroups = append(c.TestCategoryGroups[:i], c.TestCategoryGroups[i+1:]...)
}

func (c *TestCategory) DeleteSelectedTestGroupByPointer(g *TestGroup) {
	for index, step := range c.TestCategoryGroups {
		if step == g {
			c.TestCategoryGroups = append(c.TestCategoryGroups[:index], c.TestCategoryGroups[index+1:]...)
